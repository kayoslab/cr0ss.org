...
/* --------------------------------------------------------
   UI Components (moved outside main component to prevent focus loss)
--------------------------------------------------------- */
...
function GoalField({
  ...
  return (
    <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
      ...
      <Select value={period} onValueChange={(v: string) => onPeriodChange(v as GoalPeriod)}>
...
/** Labeled select using shadcn-ui Select. */
function SelectField({
  ...
  // Extract option values and labels from children
  ...
  // Find placeholder (option with empty value)
  const placeholderOption = allOptions.find(opt => opt.props.value === '');
  const placeholder = placeholderOption ? String(placeholderOption.props.children) : 'Select...';

  // Filter out empty value options (Radix UI doesn't allow empty string values)
  const options = allOptions.filter(opt => opt.props.value !== '');
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <Select value={value || undefined} onValueChange={onChange}>
        ...
        <SelectContent>
          {options.map((opt) => (
            <SelectItem key={opt.props.value} value={opt.props.value}>
              {opt.props.children}
            </SelectItem>
          ))}
        </SelectContent>
