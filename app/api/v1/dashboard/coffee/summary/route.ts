...
/** GET /api/v1/dashboard/coffee/summary?date=YYYY-MM-DD — cups and brew methods for a day. */
export const GET = createApiRoute()
  ...
  .handle(async (request) => {
    try {
      ...
      return privateJson(await getCoffeeSummary(date));
    } catch (error) {
      console.error('Error fetching coffee summary:', error);
      return internalError('Failed to fetch coffee summary', error);
    }
  });
